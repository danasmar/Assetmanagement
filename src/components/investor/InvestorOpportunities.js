   ...
   <Card style={{ cursor:'default', display:'flex', flexDirection:'column' }}>
     ...
     <div style={{ flex:1 }}>
       ...
       <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginBottom:'0.75rem' }}>
         {[['Target IRR', deal.target_irr ? `${deal.target_irr}%` : '—'], ['Min. Ticket', fmt.currency(deal.min_investment, deal.currency||'SAR')], ['Distributions %', deal.distribution_pct ? `${deal.distribution_pct}%` : '—'], ['Closing', deal.closing_date||'TBC']].map(([k,v]) => (
