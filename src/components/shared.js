...
// ─── Audi Capital Brand Mark (inline SVG) ────────────────────────────────────
const AudiCapitalMark = ({ size = 36 }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 500 500"
  ...
  >
    {/* Navy background */}
    <rect width="500" height="500" fill="#0a2157" />

    {/* Large white circle (left) */}
    <circle cx="185" cy="255" r="158" fill="white" />
    {/* Navy cutout to create left donut arc */}
    <circle cx="225" cy="225" r="98" fill="#0a2157" />

    {/* Grey overlapping circle (right) */}
    <circle cx="315" cy="255" r="132" fill="#b0aea8" opacity="0.9" />
    {/* Navy cutout to create right donut arc */}
    <circle cx="315" cy="255" r="76" fill="#0a2157" />
...
export function Layout({ children, page, onPageChange, session, onLogout, navItems }) {
  ...
  const Sidebar = ({ style }) => (
    <div style={style}>
      {/* ── Logo: graphic left + text right ── */}
      <div style={nav.logo}>
        <AudiCapitalMark size={36} />
        <div>
