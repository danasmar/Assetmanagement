...
const navStyle = {
  ...
  signOut: { display:'flex', alignItems:'center', gap:'0.5rem', color:'rgba(255,255,255,0.5)', fontSize:'0.82rem', cursor:'pointer', padding:'0.4rem 0', border:'none', background:'transparent', width:'100%', fontFamily:'DM Sans, sans-serif' },
  overlay: { position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:150 },
  topbar: { height:'56px', background:'#fff', borderBottom:'1px solid #e9ecef', display:'flex', alignItems:'center', padding:'0 1.5rem', position:'fixed', top:0, right:0, left:'240px', zIndex:99, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' },
  topbarMobile: { height:'56px', background:'#fff', borderBottom:'1px solid #e9ecef', display:'flex', alignItems:'center', padding:'0 1rem', position:'fixed', top:0, right:0, left:0, zIndex:99, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' },
  hamburger: { border:'none', background:'none', cursor:'pointer', padding:'6px', display:'flex', flexDirection:'column', gap:'5px' },
  hbar: { width:'22px', height:'2px', background:'#003770', borderRadius:'2px', display:'block' },
  content: { marginLeft:'240px', marginTop:'56px', minHeight:'calc(100vh - 56px)', background:'#f8f9fa' },
  contentMobile: { marginLeft:0, marginTop:'56px', minHeight:'calc(100vh - 56px)', background:'#f8f9fa' },
};
...
<div style={navStyle.bottom}>
  ...
  if (isMobile) return (
    <div style={{ fontFamily:'DM Sans, sans-serif' }}>
      ...
      <header style={navStyle.topbarMobile}>
        <button style={navStyle.hamburger} onClick={() => setMobileOpen(!mobileOpen)}>
          <span style={navStyle.hbar} /><span style={navStyle.hbar} /><span style={navStyle.hbar} />
        </button>
        <span style={{ fontFamily:'DM Serif Display, serif', fontSize:'1.1rem', color:'#003770', marginLeft:'0.75rem' }}>Audi Capital</span>
      </header>
      <main style={navStyle.contentMobile}>{children}</main>
    </div>
  );
...
export const Badge = ({ label, type }) => {
  ...
  return <span style={{ background:c.bg, color:c.color, padding:'3px 10px', borderRadius:'20px', fontSize:'0.72rem', fontWeight:'600' }}>{label}</span>;
};
...
export const Btn = ({ children, onClick, variant, style, disabled, type }) => {
  ...
  return (
    <button type={type||'button'} onClick={onClick} disabled={disabled}
      style={{ ...v, padding:'0.55rem 1.1rem', borderRadius:'8px', fontSize:'0.85rem', fontWeight:'600', cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.6 : 1, fontFamily:'DM Sans, sans-serif', transition:'all 0.15s', ...style }}>
      {children}
    </button>
  );
};
...
export const Modal = ({ title, onClose, children, wide }) => (
  <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
    <div style={{ background:'#fff', borderRadius:'16px', width:'100%', maxWidth: wide ? '760px' : '500px', maxHeight:'90vh', overflow:'auto', boxShadow:'0 24px 64px rgba(0,0,0,0.25)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1.25rem 1.5rem', borderBottom:'1px solid #e9ecef' }}>
